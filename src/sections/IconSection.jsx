...
const IconSection = () => {
    useEffect(() => {
        // Initialize Lenis and GSAP after component mounts
        ...
        const textSegment = document.querySelectorAll('.text-segment');
        const placeholders = document.querySelectorAll('.placeholder-icon');
        const heroHeader = document.querySelector('.hero-header');
        const heroSection = document.querySelector('.hero-icon');


        if (!animatedIcons || !iconElements.length || !textSegment.length || !placeholders.length || !heroHeader || !heroSection) {
            console.error('Required DOM elements not found');
            return;
        }

        const textAnimationOrder = []
        textSegment.forEach((segment, index) => {
            textAnimationOrder.push({
                segment, originalIndex: index
            })
        })
            ...
            onUpdate: (self) => {
                ...
                if (progress < 0.3) {
                    ...
                    textAnimationOrder.forEach((item, randomIndex) => {
                        const segmentStart = randomIndex * 0.03;
    ...
    return (
        <div className='hero-section'>
            <section id='sec' className='hero-icon'>
                <div className="hero-header">
                    <h1 className='header-h'>D-CodeHood</h1>
                    <p className='header-p'>These are some of the major brands I have worked with.</p>
                </div>

                <div className="animated-icons">
                    <div className="animated-icon icon-1">
                        <img className='img-icon' src="/images/logos/relix.png" alt="" />
                    </div>
                    <div className="animated-icon icon-2">
                        <img className='img-icon' src="/images/logos/bLogo.png" alt="" />
                    </div>
                    <div className="animated-icon icon-3">
                        <img className='img-icon' src="/images/logos/remeda2.png" alt="" />
                    </div>
                    <div className="animated-icon icon-4">
                        <img className='img-icon' src="/images/person.png" alt="" />
                    </div>
                    <div className="animated-icon icon-5">
                        <img className='img-icon' src="/images/logos/homeLogo.png" alt="" />
                    </div>
                </div>

                <div className="animated-text">
                    <span className="text-segment">From top brands like</span>
                    <div className="placeholder-icon"></div>
                    <span className="text-segment">worked on countless projects with</span>
                    <div className="placeholder-icon"></div>
                    <span className="text-segment">shaping digital ideas with</span>
                    <div className="placeholder-icon"></div>
                    <span className="text-segment">Trusted by</span>
                    <div className="placeholder-icon"></div>
                    <span className="text-segment">and other global innovators.</span>
                    <div className="placeholder-icon"></div>
